...
namespace Microsoft.Azure.Amqp
{
    ...
    sealed class AmqpEventSource : EventSource
    {
        ...
        [Event(15, Level = EventLevel.Error, Message = "{0}: open {1} failed (entity={2},error={3}).")]
        public unsafe void AmqpOpenEntityFailed(string source, string name, string entityName, string error)
        {
            fixed (char* ptrSource = source)
            fixed (char* ptrName = name)
        ...
        [Event(16, Level = EventLevel.Informational, Message = "{0}: open {1} succeeded (entity={2}).")]
        public unsafe void AmqpOpenEntitySucceeded(string source, string name, string entityName)
        {
            fixed (char* ptrSource = source)
            fixed (char* ptrName = name)
            fixed (char* ptrEntityName = entityName)
        ...
        public unsafe void AmqpCacheMessage(string source, uint deliveryId, int transfers, uint totalLinkCredit, uint linkCredit)
        {
            fixed (char* ptrSource = source)
            {
                WriteEvent(
