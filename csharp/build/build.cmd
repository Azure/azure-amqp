...
call nuget restore "%build-root%\csharp\microsoft_azure_amqp.sln"
if %build-clean%==1 (
    call :clean-a-solution "%build-root%\csharp\microsoft_azure_amqp.sln" %build-config% %build-platform%
    if not !errorlevel!==0 exit /b !errorlevel!
)
call :build-a-solution "%build-root%\csharp\microsoft_azure_amqp.sln" %build-config% %build-platform%
if not !errorlevel!==0 exit /b !errorlevel!
