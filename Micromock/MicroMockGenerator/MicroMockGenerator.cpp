...
_T("//\n")
_T("//\n")
_T("// Use of this source code is subject to the terms of the Microsoft end-user\n")
_T("// license agreement (EULA) under which you licensed this SOFTWARE PRODUCT.\n")
_T("// If you did not accept the terms of the EULA, you are not authorized to use\n")
_T("// this source code. For a copy of the EULA, please see the LICENSE.RTF on your\n")
_T("// install media.\n")
_T("//\n")
...
void GenerateMockCallMacros(_In_ size_t supportedArgCount)
{
    ...
    _tfopen_s(&outFile, _T("..\\MicroMock\\MicroMockCallMacros.h"), _T("wt"));
    if (NULL != outFile)
    {
        ...
        for (UINT currentArgCount = 0; currentArgCount <= supportedArgCount; currentArgCount++)
        {
            ...
            // hook function mock
            _ftprintf(outFile, _T("#define MOCK_HOOK_METHOD_%u(prefix, resultType, name%s) \\\n"), currentArgCount, argAndValuesMacroString.c_str());
            _ftprintf(outFile, _T("INSTALL_HOOK(::name, name) \\\n"));
            _ftprintf(outFile, _T("MOCK_ANY_METHOD_%u(static, STATIC_, prefix, resultType, name%s) \n"), currentArgCount, argAndValuesMacroString.c_str());
            _ftprintf(outFile, _T("\n"));

            // hook extended function mock
            _ftprintf(outFile, _T("#define MOCK_HOOK_METHOD_EX_%u(prefix, resultType, name%s, functionToHook) \\\n"), currentArgCount, argAndValuesMacroString.c_str());
            _ftprintf(outFile, _T("INSTALL_HOOK(functionToHook, name) \\\n"));
            _ftprintf(outFile, _T("MOCK_ANY_METHOD_%u(static, STATIC_, prefix, resultType, name%s) \n"), currentArgCount, argAndValuesMacroString.c_str());
            _ftprintf(outFile, _T("\n"));
...
void GenerateTimeDiscreteMockCallMacros(_In_ size_t supportedArgCount)
{
    ...
    ofstream fout("..\\MicroMock\\TimeDiscreteMicroMockCallMacros.h");
    fout<<CfileHeader<<endl;
    for(i=0;i<supportedArgCount;i++)
    {
        fout<<"#define MOCK_HOOK_TD_METHOD_"<<i<<"(prefix, resultType, name";
        ...
        //MOCK_HOOK_METHOD1... same thing
        fout<<tab<<"MOCK_HOOK_METHOD_"<<i<<"(prefix, resultType, name";
