namespace Test.Microsoft.Azure.Amqp
{
    ...
    public class UtilityTests
    {
        ...
        public void SerializedWorkerTest()
        {
            ...
            if (!waitOne)
            {
                Assert.IsTrue(false, "Worker did not complete in time");
            }
            else
            {
            ...
            Action<SequenceNumber, SequenceNumber> invalidCompare = (a, b) =>
                {
                    try
                    {
                        ...
                        temp = a <= b;
                        temp = a >= b;
                        Assert.IsTrue(false, "Invalid comparisons should fail");
                    }
        ...
        public void BufferListStreamTest()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(false, "Seek should fail with argument out of range exception");
            }
            ...
            {
                stream.Position = 100;
                Assert.IsTrue(false, "Stream is disposed!!");
            }
        ...
        static void TryWrite(ByteBuffer buffer, int newData, bool fail, AmqpSymbol error)
        {
            try
            {
                ...
                if (fail)
                {
                    Assert.IsTrue(false, "write should fail because buffer is smaller");
                }
            }
